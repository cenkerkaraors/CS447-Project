...
def add_CallBack():
    ...
    src_label = Label(directory_Page, text="Enter Source Location here: ").grid(row=1, column=1,)
    src_entry = Entry(directory_Page, bd=2)
    src_entry.grid(row=1, column=2)

    dest_label = Label(directory_Page, text="Enter Destination Location here: ").grid(row=2, column=1)
    dest_entry = Entry(directory_Page, bd=2)
    dest_entry.grid(row=2, column=2)

    dir_add = Button(directory_Page, text="     Add     ", fg="black",
                     command=partial(dir_CallBack, src_entry, dest_entry)).grid(row=3, column=2)

    exit_button = Button(directory_Page, text="     Exit     ", fg="black",
                         command=partial(exit_CallBack, directory_Page)).grid(row=4, column=2)
...
def init_gui():
    ...
    add_Button = Button(top_frame, text="Add", fg="black", command=add_CallBack, compound=TOP, image=img_add)
    add_Button.pack(side=LEFT)

    img_sync = PhotoImage(file='syncbutton.gif')
    sync_Button = Button(top_frame, text="Sync", fg="black", command=sync_CallBack, compound=TOP, image=img_sync)
    ...
    refresh_Button = Button(top_frame, text="Refresh", fg="black", command=partial(showDirs, frame), compound=TOP,
                            image=img_refresh)
    refresh_Button.pack()
